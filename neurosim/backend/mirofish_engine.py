...
class MiroFishEngine:
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                payload = {
                    ...
                    "prediction_requirements": content.get("requirements", "Predict audience reaction"),
    ...
    async def _simulated_simulation(
        ...
        for i in range(num_agents):
            ...
            reactions.append({
                ...
                "comment": random.choice(comment_types.get(persona["engagement_style"], ["..."])),
    ...
    async def run_what_if(
        ...
        if modifications.get("earlier_product_mention"):
            effects.append({"effect": "negative", "magnitude": 0.15, "description": "Earlier product mention reduces authenticity"})
        if modifications.get("price_decrease"):
            effects.append({"effect": "positive", "magnitude": modifications["price_decrease"] / 100 * 0.3, "description": f"Price decrease of {modifications['price_decrease']}% increases purchase intent"})
        if modifications.get("aggressive_cta"):
            effects.append({"effect": "negative", "magnitude": 0.1, "description": "Aggressive CTA triggers ad-skip behavior"})
        if modifications.get("emotional_tone"):
            effects.append({"effect": "positive", "magnitude": 0.08, "description": "Emotional tone increases engagement"})
        ...
        return {
            ...
            "predicted_outcome": {
                ...
                "risk_change": "increased" if any(m["effect"] == "negative" for m in effects) else "reduced",
            ...
            "comparison": {
                ...
                "modified_sentiment": round(base_simulation.get("final_sentiment", 50) + (positive_sum - negative_sum) * 100, 1)
            }
        }
