...
class NeuroSocialBridge:
    ...
    def calculate_share_probability(
        roi: ROI, social_multiplier: float = SOCIAL_MULTIPLIER
    ) -> float:
        return round(min(roi.Area45 * social_multiplier, 1.0), 4)

    @staticmethod
    def stage_gate_check(
        W_attn: float, threshold: float = STAGE_GATE_THRESHOLD
    ) -> Tuple[bool, str]:
        if W_attn < threshold:
            return (
                False,
                f"Low biological engagement (W_attn={W_attn:.2f} < {threshold}). Simulation aborted.",
            )
    ...
    def compute_social_params(roi: ROI) -> SocialParams:
        ...
        return SocialParams(
            W_attn=W_attn, P_skip=P_skip, P_share=P_share, viral_coefficient=viral_coef
        )
            ...
            recs.append(
                {
                    "timestamp": "0:00-0:03",
                    "type": "visual",
                    "issue": "Low visual hook strength",
                    "recommendation": "Add more visual contrast or motion in the first 3 seconds",
                }
            )
        elif roi.LO > 0.75:
            recs.append(
                {
                    "timestamp": "0:00-0:03",
                    "type": "visual",
                    "issue": "Strong visual engagement",
                    "recommendation": "Maintain current visual style - high LO activation",
                }
            )
            ...
            recs.append(
                {
                    "timestamp": "0:05-0:15",
                    "type": "audio",
                    "issue": "Audio engagement dropping",
                    "recommendation": "Consider audio dynamic change or voice modulation",
                }
            )
            ...
            recs.append(
                {
                    "timestamp": "variable",
                    "type": "cta",
                    "issue": "Low perceived value",
                    "recommendation": "Strengthen value proposition or add social proof",
                }
            )
            ...
            recs.append(
                {
                    "timestamp": "N/A",
                    "type": "social",
                    "issue": "High viral potential",
                    "recommendation": f"High Area45 ({roi.Area45}) driving viral coefficient of {social_params.viral_coefficient}",
                }
            )
